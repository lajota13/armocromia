...
class Cropper(Thread):
    def __init__(self, paths: List[str], dst_root: str, pbar: tqdm):
        self.paths = paths
...
def preprocess_season(season_dir: str, dst_root: str):
    paths = glob.glob(season_dir + "/*.png")
    chunks = split_paths(paths, WORKERS_NUM)
    with tqdm(total=len(paths)) as pbar:
        croppers = [Cropper(c, dst_root, pbar) for c in chunks]
        for c in croppers:
            c.start()
        for c in croppers:
            c.join()
...
def preprocess_dataset(src_root: str):
    ...
    for i, season_dir in enumerate(seasons_dirs, 1):
        ...
        preprocess_season(season_dir, dst_root)
